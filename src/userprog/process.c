...
process_execute (const char *file_name) 
{
  ...
  _prog_name = malloc(strlen(file_name) + 1);
  strlcpy (_prog_name, file_name, strlen(file_name)+1);
  _prog_name = strtok_r((char*) _prog_name, " ", &save_ptr );

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (_prog_name, PRI_DEFAULT, start_process, fn_copy);
  if (tid == TID_ERROR)
